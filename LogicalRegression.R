...
setwd("C:/Users/hp/Desktop/1")

dataAll = read.csv("Proactive Attrition Management-Logistic Regression Case Study.csv", header = TRUE) 
...
#Since 45% of data have missing values hence inserting these values with mean as they should not be deleted.
...
## Correlation matrix
corrmat= cor(dataAll[,num_var]) 
#View(corrmat)
write.csv(corrmat, file = "corrmat.csv")

# Categorical Variable(Significance Test)
tab = xtabs(~CHILDREN + CHURN, data = dataAll)              
chisq.test(tab)
...
chisq.test(tab)
tab = xtabs(~CREDITGY + CHURN, data = dataAll)                
chisq.test(tab)
tab = xtabs(~CREDITZ + CHURN, data = dataAll)                
chisq.test(tab)
...
chisq.test(tab)
tab = xtabs(~TRUCK + CHURN, data = dataAll)                
chisq.test(tab)
tab = xtabs(~RV + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~OCCPROF + CHURN, data = dataAll)                
chisq.test(tab)
tab = xtabs(~OCCCLER + CHURN, data = dataAll)                  
chisq.test(tab)
tab = xtabs(~OCCCRFT + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~OCCSTUD + CHURN, data = dataAll)                
chisq.test(tab)
tab = xtabs(~OCCHMKR + CHURN, data = dataAll)               
chisq.test(tab)
tab = xtabs(~OCCRET + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~OCCSELF + CHURN, data = dataAll)               
chisq.test(tab)
tab = xtabs(~OWNRENT + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~MARRYUN + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~MARRYYES + CHURN, data = dataAll)                 
chisq.test(tab)
...
chisq.test(tab)
tab = xtabs(~MAILFLAG + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~TRAVEL + CHURN, data = dataAll)                
chisq.test(tab)
tab = xtabs(~PCOWN + CHURN, data = dataAll)                
chisq.test(tab)
tab = xtabs(~CREDITCD + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~NEWCELLY + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~NEWCELLN + CHURN, data = dataAll)              
chisq.test(tab)
tab = xtabs(~INCMISS + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~INCOME + CHURN, data = dataAll)                 
chisq.test(tab)
tab = xtabs(~MCYCLE + CHURN, data = dataAll)                 
chisq.test(tab)
...
write.csv(decile_summ_dev,"dev.csv")
...
View(decile_summ1)
write.csv(decile_summ1, "value.csv")
